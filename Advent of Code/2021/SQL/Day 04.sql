...
-- Input data
------------------------------------------------------------------------------
	IF OBJECT_ID('tempdb..#calls','U') IS NOT NULL DROP TABLE #calls; --SELECT * FROM #calls
	CREATE TABLE #calls (ID int IDENTITY, Val int NOT NULL);

	IF OBJECT_ID('tempdb..#raw_card_data','U') IS NOT NULL DROP TABLE #raw_card_data; --SELECT * FROM #raw_card_data
	CREATE TABLE #raw_card_data (RowID int IDENTITY, Val varchar(20) NOT NULL);
	...
	BEGIN;
		-- Sample data
		INSERT INTO #calls (Val) VALUES
			(7),(4),(9),(5),(11),(17),(23),(2),(0),(14),(21),(24),(10),(16),(13),(6),(15),(25),(12),(22),(18),(20),(8),(19),(3),(26),(1)

		INSERT INTO #raw_card_data (Val) VALUES
	...
	CREATE TABLE #card_data (
		...
		-- Needed for phase 2
		CardWinningCallValue	int			NULL,
		CardWinningCallRound	int			NULL,
	);

	INSERT INTO #card_data (CardID, RowID, ColID, BoxValue)
	SELECT CardID	= x.CardID + 1
		, RowID		= DENSE_RANK() OVER (PARTITION BY x.CardID ORDER BY x.RowID)
		, ColID		= ROW_NUMBER() OVER (PARTITION BY x.CardID, x.RowID ORDER BY 1/0)
		, BoxValue	= CONVERT(int, y.[value])
	FROM (
		SELECT cd.RowID
			, CardID = SUM(IIF(cd.Val = '', 1, 0)) OVER (ORDER BY cd.RowID)
			, cd.Val
		FROM #raw_card_data cd
	) x
		CROSS APPLY STRING_SPLIT(RTRIM(LTRIM(REPLACE(x.Val,'  ',' '))), ' ') y -- Bad practice, but relying on order returned
	WHERE y.[value] <> ''
------------------------------------------------------------------------------
...
-- Find all winners
------------------------------------------------------------------------------
	DECLARE @CallValue int, @RoundID int = 0;

	DECLARE ff_cursor CURSOR FAST_FORWARD LOCAL
		FOR SELECT c.Val FROM #calls c ORDER BY c.ID
	OPEN ff_cursor
	...
	BEGIN;
		...
		FETCH NEXT FROM ff_cursor INTO @CallValue;

		IF (@@FETCH_STATUS <> 0) BREAK;

		-- Mark off called numbers
		UPDATE cd SET Marked = 1
		FROM #card_data cd
		WHERE cd.BoxValue = @CallValue
			AND cd.Marked = 0
			AND cd.CardWinningCallRound IS NULL

		-- Mark off winning cards
		UPDATE cd SET cd.CardWinningCallValue = @CallValue, cd.CardWinningCallRound = @RoundID
		FROM #card_data cd
		WHERE cd.CardWinningCallRound IS NULL
			AND (
				   EXISTS (SELECT x.CardID FROM #card_data x WHERE x.Marked = 1 AND x.CardID = cd.CardID GROUP BY x.CardID, x.RowID HAVING COUNT(*) = 5)
				OR EXISTS (SELECT x.CardID FROM #card_data x WHERE x.Marked = 1 AND x.CardID = cd.CardID GROUP BY x.CardID, x.ColID HAVING COUNT(*) = 5)
			)
	END;
	...
	IF OBJECT_ID('tempdb..#result','U') IS NOT NULL DROP TABLE #result; --SELECT * FROM #result
	SELECT cd.BoxValue, cd.CardWinningCallValue
		, WinnersASC	= DENSE_RANK() OVER (ORDER BY cd.CardWinningCallRound, cd.CardID)
		, WinnersDESC	= DENSE_RANK() OVER (ORDER BY cd.CardWinningCallRound DESC, cd.CardID)
	INTO #result
	FROM #card_data cd
	WHERE cd.Marked = 0
------------------------------------------------------------------------------
...
-- Part 1
------------------------------------------------------------------------------
	SELECT Answer = SUM(r.BoxValue) * MAX(r.CardWinningCallValue)
	FROM #result r
	WHERE r.WinnersASC = 1
------------------------------------------------------------------------------
