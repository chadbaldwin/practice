	...
	FROM #rawdata r
		...
		CROSS APPLY (
			SELECT Direction = CASE
							...
							WHEN x1 < x2 AND y1 > y2		 THEN 'D-DR' -- 'Diag - down to right'
							WHEN x1 > x2 AND y1 < y2		 THEN 'D-UL' -- 'Diag - up to left'
							WHEN x1 < x2 AND y1 < y2		 THEN 'D-UR' -- 'Diag - up to right'
							WHEN x1 > x2 AND y1 > y2		 THEN 'D-DL' -- 'Diag - down to left'
							ELSE NULL
						END
		) x
