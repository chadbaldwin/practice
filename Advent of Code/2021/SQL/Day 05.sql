...
------------------------------------------------------------------------------
    ...
    -- Determine max size of plane
    SELECT @MaxX = MAX(x.x), @MaxY = MAX(y.y)
    FROM #data d
        CROSS APPLY (SELECT x.x FROM (VALUES (d.x1),(d.x2)) x(x)) x
        CROSS APPLY (SELECT y.y FROM (VALUES (d.y1),(d.y2)) y(y)) y;

    IF OBJECT_ID('tempdb..#coordinates','U') IS NOT NULL DROP TABLE #coordinates; --SELECT * FROM #coordinates ORDER BY ID
    ...
    SELECT x = x.rn, y = y.rn
    FROM c4 x
        JOIN c4 y ON y.rn <= @MaxY
    WHERE x.rn <= @MaxX
    ORDER BY x.rn, y.rn;
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
    /*
        Painfully slow...probably because we're generating a cartesian product of ~312M records
        and then finding all overlapping coordinates. There's probably a less brute force way
        to calculate this, but I don't know the math.

        Could probably get the answer through some sort of calculation by comparing the
        coordinates themselves to each other and it would be significantly less than ~312M
    */

    SELECT Answer = COUNT(x.OverlapCount)
    FROM (
        SELECT c.ID, OverlapCount = COUNT(*)
        FROM #coordinates c
            JOIN #data d ON ((c.x >= d.x1 AND c.x <= d.x2) AND (c.y >= d.y1 AND c.y <= d.y2))
                        OR  ((c.x >= d.x2 AND c.x <= d.x1) AND (c.y >= d.y2 AND c.y <= d.y1))
        WHERE LEFT(d.Direction, 1) IN ('V','H') -- Horizontal or Veritcal lines only
        GROUP BY c.ID
    ) x
    WHERE x.OverlapCount > 1;
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
    /*
        This is even painfully slower than Part 1 (no surprise)...took 2 minutes to run on my SQL container.
    */
    SELECT Answer = COUNT(x.OverlapCount)
    FROM (
        SELECT x.ID, OverlapCount = COUNT(*)
        FROM (
            SELECT c.ID
            FROM #data d
                JOIN #coordinates c ON (c.x BETWEEN d.x1 AND d.x2) AND (c.y BETWEEN d.y1 AND d.y2)
            WHERE LEFT(d.Direction, 1) IN ('V','H')
                OR (d.Direction = 'D-UR' AND c.x - c.y = d.x1 - d.y1)
            UNION ALL
            SELECT c.ID
            FROM #data d
                JOIN #coordinates c ON (c.x BETWEEN d.x1 AND d.x2) AND (c.y BETWEEN d.y2 AND d.y1)
            WHERE (d.Direction = 'D-DR' AND c.x + c.y = d.x1 + d.y1)
        ) x
        GROUP BY x.ID
    ) x
    WHERE x.OverlapCount > 1;

    --SELECT d.ID, d.OriginalString, d.Direction
    --  , d.x1, d.y1, d.x2, d.y2
    --  , c.x, c.y
    --  , '|||||'
    --  , CONCAT('(', d.x1, ',', d.y1, ') -> (', d.x2, ',', d.y2, ')')
    --  , CONCAT('(', c.x, ',', c.y, ')')
    --FROM #data d
    --  LEFT JOIN #coordinates c ON (c.x BETWEEN d.x1 AND d.x2) AND (c.y BETWEEN d.y2 AND d.y1)
    --WHERE d.Direction = 'D-DR' AND c.x + c.y = d.x1 + d.y1
    --ORDER BY d.ID
------------------------------------------------------------------------------
