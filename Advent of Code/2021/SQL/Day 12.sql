...
------------------------------------------------------------------------------
	...
	*/
	...
	/*
		...
		Join all links whose Start matches the last value to replace the old list:
		'start-A-c'		, 'c'
		'start-A-b'		, 'b'
		'start-A-end'	, 'end'
		'start-b-A'		, 'A'
...
-- Part 1
------------------------------------------------------------------------------
	IF OBJECT_ID('tempdb..#paths','U') IS NOT NULL DROP TABLE #paths; --SELECT * FROM #paths
	CREATE TABLE #paths (
	...
	BEGIN;
		...
		USING (
			...
			WHERE p.LastCave <> 'end' -- Only add to paths without an end
				AND IIF(CHARINDEX(','+d.PathEnd+',', p.FullPath) > 0 AND d.PathEnd = LOWER(d.PathEnd), 0, 1) = 1 -- ignore adding new paths that are illegal
		) n ON 1=0
