...
------------------------------------------------------------------------------
    ...
    SELECT g.GameID, r.Val
        , RMax = MAX(IIF(z.Color = 'red', z.ColorCount, NULL))
        , GMax = MAX(IIF(z.Color = 'green', z.ColorCount, NULL))
        , BMax = MAX(IIF(z.Color = 'blue', z.ColorCount, NULL))
    INTO #data
    FROM #rawdata r
        CROSS APPLY (SELECT GameID = CONVERT(int, SUBSTRING(r.Val, 6, CHARINDEX(':', r.Val)-6))) g
        OUTER APPLY (
            SELECT GameResults = LTRIM(RTRIM([value]))
            FROM STRING_SPLIT(SUBSTRING(r.Val, CHARINDEX(':', r.Val)+2, LEN(r.Val)), ';')
        ) x
        CROSS APPLY (
            SELECT ColorCount = LTRIM(RTRIM([value]))
            FROM STRING_SPLIT(x.GameResults, ',')
        ) y
        CROSS APPLY (
            SELECT ColorCount = CONVERT(int, LEFT(y.ColorCount, CHARINDEX(' ', y.ColorCount)))
                ,  Color      = SUBSTRING(y.ColorCount, CHARINDEX(' ', y.ColorCount)+1, 100)
        ) z
    GROUP BY g.GameID, r.Val
------------------------------------------------------------------------------

------------------------------------------------------------------------------
-- Part 1
------------------------------------------------------------------------------
    SELECT Answer = SUM(x.GameID)
    FROM #data x
    WHERE x.RMax <= 12 AND x.GMax <= 13 AND x.BMax <= 14
------------------------------------------------------------------------------

------------------------------------------------------------------------------
-- Part 2
------------------------------------------------------------------------------
    SELECT Answer = SUM(x.RMax * x.GMax * x.BMax)
    FROM #data x
------------------------------------------------------------------------------
